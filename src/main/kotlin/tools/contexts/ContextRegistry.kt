...
//            println("Processing counter Contexts: ${processingContexts.get()}")
    ...
    private suspend fun processContext(context: Context) {
        ...
        for (incompleteContext: Context in possibleContexts) {
            ...
            if (commonAncestors.isNotEmpty() || commonAncestorsAreTheSameOrHeads(context, incompleteContext, commonAncestors)) {
    ...
    private fun commonAncestorsAreTheSameOrHeads(
        ...
        val ancestorsAreTheSame = ancestors1.zip(ancestors2).all { (ancestor1, ancestor2) -> ancestor1?.equals(ancestor2) ?: false}
